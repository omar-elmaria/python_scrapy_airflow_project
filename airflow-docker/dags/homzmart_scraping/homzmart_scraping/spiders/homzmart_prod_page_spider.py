...
try:
    with open('/opt/airflow/data/Output_SubCat_Page.json', 'r') as file: # ADDED /opt/airflow/data to tell Airflow where to look for the JSON file
        data = json.load(file)
except FileNotFoundError:
    with open('/home/oelmaria/python_projects/python_scrapy_project/airflow-docker/data/Output_SubCat_Page.json', 'r') as file: # If you are using the local env, the interpreter will look for the JSON file through a different directory
        data = json.load(file)
